...
class GKTemplate {
    ...
    // function to get tools override
    public function getPageTitleOverride() {
    	// get current ItemID
    	$ItemID = JRequest::getInt('Itemid');
    	// get current option value
    	$option = JRequest::getCmd('option');
    	// override array
    	$page_title_override = $this->config->get('page_title_override');
    	// get current tools setting
        $page_title_mode = $this->API->get('page_title', 1);
        //
        if($page_title_mode == 1) { 
        	return 
        		!(
	        		(isset($page_title_override[$ItemID]) && $page_title_override[$ItemID] == 'off')
	        		||
	        		(isset($page_title_override[$option]) && $page_title_override[$option] == 'off')
        		)
        	; 
        } else if($page_title_mode == 0) { 
            return  
            	(isset($page_title_override[$ItemID]) && $page_title_override[$ItemID] == 'on')
            	||
            	(isset($page_title_override[$option]) && $page_title_override[$option] == 'on')
            ; 
        }
    }
}
